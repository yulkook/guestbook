...
pipeline {
    ...
    environment {
        ...
        strDockerImage ="yulkooklee/cicd_guestbook:${strDockerTag}"
    }
    ...
    stages {
        stage('Checkout') {
            agent { label 'agent1' }
            steps {
                git branch: 'master', url:'https://github.com/yulkook/guestbook.git'
            }
        }
        stage('Build') {
            agent { label 'agent1' }
        ...
        stage('Docker Image Push') {
            ...
            steps {
                script {
                    docker.withRegistry('', 'DockerHub_yulkooklee') {
                        oDockImage.push()
                    }
                }
            }
